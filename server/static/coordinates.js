...
class Coordinates extends EventEmitter {
    ...
    constructor(id) {
        ...
        let x= L.control.calendar({position: 'topright'}).addTo(this.mymap);
    ...
    init_trail_button() {
        L.easyButton('fa fa-arrow-up', (btn, map) => {
...
class Compass {
    ...
    updateCompass = (my_position, currentPosition) => {
        if (this.my_position) {
            ...
            console.log(bearing);
            $("#compass_div").css({'transform' : 'rotate('+ (bearing) +'deg)'});
        }
    }
}
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.length > 0) {
                    ...
                    if (thisCopy.last_ts && data[p]['ts'] <= thisCopy.last_ts) {
                    ...
                    if (thisCopy.currentPosition != newPosition) {
                        ...
                        if (l == 0 ||  thisCopy.segments[l-1].getLatLngs().length >= 100) {
                            if (this.follow_trail) {
                                while (thisCopy.segments.length*100 > this.trail_length) {
                                    let s = thisCopy.segments.shift();
                                    thisCopy.mymap.mymap.removeLayer(s); 
                                }
                            }
                        ...
                        thisCopy.emit('tracker_position', [ thisCopy.imei, newPosition, thisCopy ]);
